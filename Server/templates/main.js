...
function Head(table, data) {
    ...
    var dates = data.lessons.map(x => new Date(x[0].substr(5, 4), x[0].substr(3, 2), x[0].substr(0, 2), x[0].substr(11, 2), x[0].substr(14, 2)));
    console.log(dates);
    ...
    var months = dates.map(x => x.getMonth()).filter((x, i, a) => a.indexOf(x) === i);
    var months_style = months.map(x => [
        ["colspan", dates.filter(y => y[1] == x).length]
    ]);
    ...
    var days = dates.map(x => x.getDate());
...
function tableHandler2(data) {
    ...
    for (var key in data.students) {
        ...
        var c = student.visitations.map(x => {
            if (x == "0") return [
                ["class", "not_visited"]
            ];
            else if (x == "-1") return [
    ...
    var myChart2 = new Chart(ctx2, {
        ...
        data: {
            ...
            datasets: [{
                ...
                data: raspr,
                backgroundColor: 'rgba(31, 169, 244, 0.3)',
